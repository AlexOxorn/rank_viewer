...
namespace unleashed {
    ...
    interpret_ret interpret_data(unleashed_process& process) {
        ...
        if (is_night) {
            scores[RingBonus] = {.score = ring_bonus, .name = "Rings", .foreground = ox::named_colors::gold};
        } else {
        ...
        score_data enemy{.score = (i32) level_data.EnemyScore, .name = "Enemy", .foreground = ox::named_colors::maroon};

        if (name.contains("BossFinal")) {
            scores[EnemyBonus] = enemy;
        }
        ...
        if (is_night or name.contains("Eggman")) {
            ...
            scores[CrushBonus] = {.score = (i32) (night ? night->CrushScore : 0),
                                  .name = "Crush",
                                  .foreground = ox::named_colors::DarkViolet};
        } else {
            scores[TrickBonus] = {
                    .score = (i32) level_data.TrickScore, .name = "Tricks", .foreground = ox::named_colors::DarkViolet};
            ...
            if (level_data.MaxSpeedCount == 0) {
                ...
                auto sum = std::ranges::fold_left(level_data.Speeds, 0., std::plus<>());
                ...
                scores[SpeedBonus1] = {.score = speed_score - partial_score, .name = "Speed", .foreground = c};
            }
        };
