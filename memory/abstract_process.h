...
protected:
    ...
    [[nodiscard]] virtual bool wrong_endian() const = 0;
...
public:
    explicit abstract_process(int _pid)
        : pid(_pid){};
    ...
    template<typename T>
    requires std::is_trivially_copyable<T>::value int read_memory(u64 address, T* result) {
    ...
    requires std::is_trivially_copyable<T>::value int read_memory(u64 address, std::array<T, N>* result) {
    ...
    template<typename T>
    requires std::is_trivially_copyable<T>::value int write_memory(u64 address, T* result) {
    ...
    template<typename T, std::size_t N>
    requires std::is_trivially_copyable<T>::value int write_memory(u64 address, std::array<T, N>* result) {
        bool swap = should_endian_swap<T>();
        if (swap) ox::bswap(result);
